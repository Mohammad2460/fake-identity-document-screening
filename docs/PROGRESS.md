...
2026-09-16 — Task 20 (user-found gap): the watchlist now screens the typed name AND the names in the passport/visa MRZ, deduped, with the source named in the message. Verified: typed 'Rakesh Verma' on a passport printing MOHAMMAD ZAID -> REJECT 82 with critical WL_MATCH naming the passport MRZ. 381 tests. NEXT: rehearsals; optional face-watchlist gallery (B) then T17 batch.
2026-09-16 — Task 21: every signal now carries a plain-language line; the Why cards lead with it and hide the forensic sentence behind a per-card 'Technical detail' toggle. Controller reworded the crossdoc plain lines to name the real sources ('the passport' vs 'the details typed into the form'). 387 tests. NEXT: batch mode (T17), then face-watchlist gallery (B), rehearsals, final review.
2026-09-16 — Task 22 (stage documents): scripts/make_demo_passport.py now builds a whole set from one consenting photo — genuine passport + matching visa, plus one forgery per attack the PS names (DOB retyped, surname retyped, MRZ digit changed, photo swapped, visa entry stamp added after issue, visa issued against another passport). Passport/visa numbers are derived from the holder's name so two people's sets never collide in the case database. `--out data/demo_<name>` builds a second person alongside the first. The watchlist engine now also reads an optional gitignored `data/watchlist.local.csv`, so a consenting teammate can stand in for a wanted traveller without their name being committed as a sanctioned person. Measured per fresh DB: genuine pair CLEAR 6; DOB REVIEW 45; surname REVIEW 50; MRZ REVIEW 33; photo swap REVIEW 50; forged stamp REVIEW 33; wrong-passport visa REJECT 65; a watchlisted holder's genuine passport REJECT 65 on WL_MATCH alone, rising to 96 with MRZ tampering. Two code-review rounds (11 findings) fixed. Demo ordering now documented: genuine pair first, name forgery last, delete cases.db between rehearsals. 398 tests. NEXT: rehearsals; SIDTD accuracy measurement; optional face-watchlist gallery (B) and T17 batch mode; final whole-branch review.
2026-09-16 — Task 23 (face watchlist, the deferred "B"): new `facewatch` engine screens the document portrait and the live selfie separately against a gallery of wanted faces (`data/face_watchlist.csv`, plus a gitignored `.local.csv` overlay reusing `watchlist.py`'s `local_overlay_path` helper), reusing SFace's thresholds (critical at 0.363, a "possible match" medium band at 0.30). Catches a wanted person on a cleanly forged passport in a name that's never listed, which the text-only name watchlist structurally cannot. Silent on a missing gallery, a missing/undecodable row, or a row with no detectable face; cache keyed on the CSV's own mtime so editing the gallery takes effect without a restart (the name watchlist's path-only cache has already cost a debugging session once). Brief's suggested seed crops (`sfhq_03/05/06`) turned out wrong on inspection: `sfhq_03` is already the demo-builder's stage photo, and `sfhq_07` coincidentally scores 0.38-0.43 against `sfhq_03`/`sfhq_04` — above the match threshold, which broke two demo-passport tests until caught. Gallery shipped with two crops instead of three: `sfhq_05` (Nadia Husseini Farah, SDN) and `sfhq_06` (Marko Antun Horvat, PEP), both measured well under 0.30 against every other committed SFHQ crop (highest observed 0.207). 409 tests. NEXT: T17 batch mode (optional); rehearsals; final whole-branch review.
2026-09-17 — Housekeeping: a real teammate's name had been committed to `data/watchlist.csv` again (it belongs only in the gitignored `data/watchlist.local.csv` overlay, which already carried it — the committed row was both a duplicate and a breach of hard rule 1). Removed; verified the overlay still raises critical WL_MATCH on that name, so the stage demo is unaffected. The committed watchlist is back to eight invented names. 418 tests. NEXT: T17 batch mode (optional); SIDTD accuracy measurement; 2 timed rehearsals; final whole-branch review.
